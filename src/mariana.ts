...
import {parse, interpolate, formatRgb} from 'culori'

// Colors extracted from Mariana.sublime-color-scheme.
// Some computed colored are generated by color-mod.ts, to re-generate them, run `npm run mod`.
...
const blend = (a: string, b: string, p: number) => formatRgb(interpolate([a, b])(1 - p))
const a = (color: string, p: number) => `color-mix(in srgb, ${color} ${p * 100}%, transparent)`
